    ...
    for (const path of await peer.getFiles(false)) {
        ...
        if (!status.done) {
            ...
            t.push([
                ...
                status.speedF,
                formatDuration(Math.floor(status.timeLeft)),
                // JSON.stringify(status.partsHave) (fun for debugging)
            ]);
        }
    }
    const s = `Active transfers (${peer.downloadsActive}): Downloaded: ${(await peer.getFiles(true)).length}/${(await peer.getFiles()).length} files. Connected to ${Object.keys(peer.clients).length} peers. Total speed: ${formatBytes(speed)}/s\n` + t.toString();
...
async function main(listpath, seed = false) {
    ...
    if (listpath.startsWith('qsl:')) {
        ...
        listhash = listpath.split(':')[2];
        const client = new DFN.Client('wss://' + serverAdd);
        await client.connect();
        list = await client.requestFileDownloadList(listhash);
    } else {
        const listbuf = await fs.promises.readFile(listpath);
    ...
    peer.events.on('download-complete', (p) => {
        endReport += `Downloaded ${p.path} (${p.totalF}) in ${formatDuration(Math.floor(p.time))} (${p.speedF})\n`;
    ...
    if (seed) {
        ...
        if (!peer.cloudflared.hostname) {
            console.log('Waiting quick share link...');
            await peer.cloudflared.waitForHostname();
        }
        console.log(`Quick share link: qsl:${peer.cloudflared.hostname}:${listhash}`);
    } else {
        await peer.stop();
    }
}
