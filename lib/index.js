...
class DownloadProgressReport {
    constructor(file) {
        ...
        this.speed = (now - file.start) > 0 ? (file.receivedSinceStart / (now - file.start)) * 1000 : Infinity;
    ...
    class FullPeer extends BasePeer {
        ...
        _makeProgress(key) {
            return new DownloadProgressReport({
                ...
                hashId: this.files[key].hashId,
                size: this.files[key].info.size,
                partSize: this.files[key].info.partSize,
                received: this.files[key].received,
                receivedSinceStart: this.files[key].received - this.files[key].downloadStartBytes,
                start: this.files[key].downloadStart,
                progress: this.files[key].progress
            });
        }
                                ...
                                for (const client in this.clients) {
                                    if (this.clients[client].client.socket.open && this.clients[client].files[this.files[file].hashId] && this.clients[client].files[this.files[file].hashId][index] && this.clients[client].running_part_requests < 2) { // 2 requests at a time seems the optimal amount for speed and throughput
                                        if (!this.files[file].downloadStart) {
                                            ...
                                            this.events.emit('download-started', this._makeProgress(file));
                                        }
                                        ...
                                        setImmediate(async () => {
                                            try {
                                                ...
                                                delete this.files[file].partsActive[index];
                                                this.events.emit('download-progress', this._makeProgress(file));
                                                ...
                                                if (fileComplete) {
                                                    ...
                                                    this.events.emit('download-complete', this._makeProgress(file));
                                                    await this.announce();
                                                }
                                            } catch (e) {
